 ...
 *
 * @param {Array of Objects} records - [{id,ts,val},...]
 * @param {Array} idarr ['POO1.temp1','POO1.temp2']
 * @return {Array of Arrays} : [[1608574986578, null,42],[1608574986580, 24,null],...]
 */

function recordsForTrendRaw(records, idarr) {
  if (!idarr || !idarr.length || !records || !records.length) return [];

  // const dArr = idarr.map(item => item.split('.')[0]);
  const dArr = idarr;
  const rarr = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (!rec || !rec[2] || !rec[3] || !rec[1]) continue;

    const dn_indx = dArr.findIndex(el => el == rec[2]);
    ...
    data[1 + dn_indx] = Number(rec[3]);
    rarr.push(data);
  }
  return rarr;
}

function recordsForRaw (records, queryObj) {
  ...
  return records.map(item => {
    let splitted = idarrObj[item[2]].split('.');
    return {ts :item[1], dn: splitted[0], prop: splitted[1], val:item[3]}
  })
}
