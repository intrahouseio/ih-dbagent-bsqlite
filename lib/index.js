...
module.exports = async function(channel, opt, logger) {
  ...
  try {
    ...
    //await client.pragma('PRAGMA synchronous = NORMAL;');

    await client.createTable(getCreateTableStr('records'), 'records');
    await client.exec('CREATE INDEX IF NOT EXISTS idx_records_ts ON records (ts);');
    await client.exec('CREATE INDEX IF NOT EXISTS idx_records_dn ON records (dn);');

    await client.createTable(getCreateTableStr('timeline'), 'timeline');
    await client.exec('CREATE INDEX IF NOT EXISTS idx_records_start ON timeline (start);');
  ...
  async function write(id, payload, table) {
    ...
    const query = 'INSERT INTO ' + tableName + ' (' + columns.join(',') + ') VALUES (@dn, @prop, @ts, @val)';
 
    try {
      client.insert(query, payload);
...
async function read(id, queryObj) {
    ...
    try {
      ...
      const result = client.read(queryStr);
       const endTime = Date.now();
      ...
      if (queryObj.data_type == 'calculation') {
        payload = await calculate(queryObj, dnarr, result.map(item => ({id: item[0], dn: item[2], prop: item[3], ts: item[1], val: item[4]})));
      } else {
        payload = queryObj.target == 'trend' ? formForTrend(result) : result.map(item => ({id: item[0], dn: item[2], prop: item[3], ts: item[1], val: item[4]}));
      }
      logger.log("Payload" +  util.inspect(result));
    ...
    function formForTrend(res) {
      return dnarr.length == 1 ? res.map(item => [item[1], Number(item[4])]) : utils.recordsForTrendRaw(res, dnarr);
    }
  }
