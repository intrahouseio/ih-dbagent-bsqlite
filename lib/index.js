...
module.exports = async function(channel, opt, logger) {
  ...
  async function write(id, payload, table) {
    ...
    try {
      await client.insert(query, payload);
  ...
  async function read(id, queryObj) {
    ...
    try {
      ...
      const result = await client.read(queryStr);
       const endTime = Date.now();
    ...
    function formForTrend(res) {
      return dnarr.length == 1 ? res.map(item => [item.ts, Number(item.val)]) : utils.recordsForTrendRaw(res, dnarr);
    }
  }
