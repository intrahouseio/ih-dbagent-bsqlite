...
module.exports = async function(channel, opt, logger) {
  ...
  try {
    ...
    await client.exec('CREATE INDEX IF NOT EXISTS idx_records_idts ON records (id, ts);');
   ...
   * @param {Array of Objects} payload - [{ id, dn, prop, ts, val }]
   */
  async function write(id, payload, table) {
    ...
    query += tableName == 'records' ? '(@id, @ts, @val)': '(@dn, @prop, @start, @end, @state)';
    logger.log('query=' + query, 2);
    try {
  ...
  async function deletePoints(tableName, archDay, arrDnProp) {
    ...
    while (arrDnProp.length>0) {
      ...
      let values = chunk.map(i => `(dn='${i.id}')`).join(' OR ');
...
function getCreateTableStr(tableName) {
  ...
  switch (tableName) {
    ...
    default:
      result = 'uid INTEGER PRIMARY KEY NOT NULL,ts INTEGER NOT NULL, id INTEGER NOT NULL, val REAL';
  }
